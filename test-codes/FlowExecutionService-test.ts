...
export class FlowExecutionService {
  ...
  async executeFlow(version: RuleVersion, inputs: Record<string, any>, req?: any, res?: any): Promise<Record<string, any>> {
    try {
      ...
      const nodeExecutionCounts: Map<string, number> = new Map();
      const nodeResults: Record<string, any> = {};
      const executionPaths: Map<string, Set<string>> = new Map();

      const getExecutableNodes = () => {
        // If no nodes have been executed yet, only start nodes are executable
        if (nodeExecutionCounts.size === 0) {
          const startNodes = version.flowConfig.nodes.filter(node => node.type === 'start');
          startNodes.forEach(node => executionPaths.set(node.id, new Set([node.id])));
          return startNodes;
        }
        ...
        for (const [executedNodeId, executionCount] of nodeExecutionCounts.entries()) {
          ...
          const currentPath = executionPaths.get(executedNodeId) || new Set([executedNodeId]);
          ...
          for (const connection of outgoingConnections) {
            ...
            for (const target of targetNodes) {
              ...
              // Get the current execution count for this target node
              const targetExecutionCount = nodeExecutionCounts.get(targetNodeId) || 0;
              
              // Check if this node is part of a valid execution path
              const newPath = new Set(currentPath);
              newPath.add(targetNodeId);
              
              // Allow re-execution if the node is part of a new path
              const shouldExecute = !currentPath.has(targetNodeId) || 
                                  (currentPath.has(targetNodeId) && this.isValidReexecution(targetNodeId, connection, version.flowConfig.connections));
              ...
              if (!this.areInputNodesExecuted(inputNodes, nodeExecutionCounts)) continue;
              
              executionPaths.set(targetNodeId, newPath);
      ...
      while (iterationCount < maxIterations) {
        ...
        const executionPromises = executableNodes.map(async (node) => {
          const currentCount = nodeExecutionCounts.get(node.id) || 0;
          nodeExecutionCounts.set(node.id, currentCount + 1);
          ...
          // Store results with execution count
          if (!nodeResults[node.id]) {
            nodeResults[node.id] = [];
          }
          nodeResults[node.id].push(nodeResult);
  ...
  private areInputNodesExecuted(inputNodes: string[], executionCounts: Map<string, number>): boolean {
    return inputNodes.every(nodeId => executionCounts.has(nodeId));
  }

  private isValidReexecution(nodeId: string, currentConnection: any, connections: any[]): boolean {
    // Check if this node is part of a valid circular path or parallel execution
    const incomingConnections = connections.filter(conn => {
      if (Array.isArray(conn.to)) {
        return conn.to.some(t => t.node === nodeId);
      }
      return conn.to.node === nodeId;
    });

    // Allow re-execution if there are multiple incoming paths
    if (incomingConnections.length > 1) return true;

    // Check if this is part of an intended circular path
    const isCircularPath = this.findCircularPath(nodeId, connections);
    return isCircularPath;
  }

  private findCircularPath(startNodeId: string, connections: any[]): boolean {
    ...
    const dfs = (nodeId: string): boolean => {
      ...
      const outgoingConnections = connections.filter(conn => {
        if (Array.isArray(conn.from)) {
          return conn.from.some(source => source.node === nodeId);
        }
        return conn.from.node === nodeId;
      });
